...
1. Explain what a token is used for. used to get the authentication and payload
2. What steps can you take in your web apps to keep your data secure?use the authentication 
3. Describe how web servers work. We have a main location that branches out to other locations. 
4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers. axios.create => post, axios.read => get, axios.update => put, axios.delete => delete